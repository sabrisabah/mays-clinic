...
class MealItem(models.Model):
    ...
    # True only for a سناك1/سناك2 item the system generated itself from
    # NutritionAISettings.default_snack*_food/calorie_pct (views._sync_auto_snack)
    # — its quantity is kept in sync with the plan's calorie_target every
    # time the plan is saved. The moment a doctor edits this item's food or
    # quantity herself (MealItemDetailView.put), it flips to False and is
    # never auto-touched again — from then on it's a completely ordinary,
    # manually-owned item. Always False for every other item (manually
    # added, or AI-generated).
    is_auto_snack = models.BooleanField(default=False)
...
class NutritionAISettings(models.Model):
    ...
    # Fixed سناك1/سناك2 content applied automatically to every plan — NOT
    # generated by the AI at all (excluded from its allowed meal types in
    # services/nutrition_ai/schemas.py's AI_SELECTABLE_MEAL_TYPES). Doctor
    # request: snacks kept coming back inconsistent/problematic from AI
    # generation, so they're now a single clinic-wide default the doctor
    # sets once here (same food for every patient) — but the QUANTITY isn't
    # fixed; it's a percentage of that specific patient's own calorie_target
    # (e.g. 5% of a 2000-kcal plan vs. 5% of a 1500-kcal plan give different
    # quantities of the same food), recomputed by views._sync_auto_snack
    # every time the plan's calorie target is saved
    # (NutritionPlanDetailView.put) — see MealItem.is_auto_snack for how a
    # doctor's manual override of that item's quantity/food stops it from
    # being auto-updated again. Left blank (food=None or pct=None) means "no
    # default" — new plans get an empty سناك1/سناك2 row exactly like before
    # this feature existed.
    default_snack1_food = models.ForeignKey(
        Food, null=True, blank=True, on_delete=models.SET_NULL, related_name="+",
        help_text="الصنف الافتراضي لوجبة سناك 1 — يُضاف تلقائياً لكل خطة جديدة. اتركه فارغاً لعدم إضافة أي شيء تلقائياً.",
    )
    default_snack1_calorie_pct = models.FloatField(
        null=True, blank=True,
        help_text="نسبة سناك 1 من مجموع السعرات اليومية المستهدفة للخطة (مثال: أدخل 5 لتعني 5%) — الكمية تُحسب تلقائياً لكل مريضة حسب هدفها الخاص.",
    )
    default_snack2_food = models.ForeignKey(
        Food, null=True, blank=True, on_delete=models.SET_NULL, related_name="+",
        help_text="الصنف الافتراضي لوجبة سناك 2 — يُضاف تلقائياً لكل خطة جديدة. اتركه فارغاً لعدم إضافة أي شيء تلقائياً.",
    )
    default_snack2_calorie_pct = models.FloatField(
        null=True, blank=True,
        help_text="نسبة سناك 2 من مجموع السعرات اليومية المستهدفة للخطة (مثال: أدخل 5 لتعني 5%) — الكمية تُحسب تلقائياً لكل مريضة حسب هدفها الخاص.",
    )
