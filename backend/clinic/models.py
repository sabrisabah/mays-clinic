...
class NutritionAISettings(models.Model):
    ...
    # Fixed سناك1/سناك2 content applied automatically to every NEW plan at
    # creation time (see views._create_plan_with_meals) — NOT generated by
    # the AI at all (excluded from its allowed meal types in
    ...
    # sets once here, applied to every patient, and still freely editable
    # per-patient afterward through the normal manual meal-item builder —
    # this only decides what a brand-new plan STARTS with. Left blank
    # (food=None) means "no default" — new plans get an empty سناك1/سناك2
    # row exactly like before this feature existed.
    ...
    default_snack1_quantity = models.FloatField(null=True, blank=True, help_text="الكمية (بوحدة الصنف نفسها)")
    ...
    default_snack2_quantity = models.FloatField(null=True, blank=True, help_text="الكمية (بوحدة الصنف نفسها)")
