...
12. خذ بعين الاعتبار مدة الخطة المعتمدة (plan_duration ضمن generation_request — تحتوي duration_value وduration_unit، مثل duration_value=11 وduration_unit="شهر" وتعني حرفياً 11 شهراً كاملة وليس 11 يوماً؛ duration_total_days_approx يعطيك نفس المدة محسوبة تقريبياً بالأيام لتفادي أي التباس). هذا القالب يمثّل يوماً نموذجياً يتكرر طوال هذه المدة، فكلما طالت المدة (شهر فأكثر) كن أكثر حرصاً على اقتراح بدائل متنوعة (alternative_text) لكل صنف حتى لا يضطر المريض لتناول نفس الطبق بالضبط يومياً لفترة طويلة؛ للمدد القصيرة (يوم أو أسبوع) التنويع أقل إلحاحاً. إذا كانت المدة طويلة (شهرين فأكثر تقريباً) مع عجز حراري ملحوظ، أضف تحذيراً في warnings يوصي بإعادة تقييم الوزن/الخطة على فترات (وليس فقط قبل نهاية المدة الطويلة) قبل استمرار نفس الخطة لهذه المدة الطويلة كاملة.
...
الحقل الوحيد الذي يمثل تفضيلات حقيقية يمكن مراعاتها هو "doctor_instructions" الصادر من الطبيب نفسه — وحتى هو لا يتجاوز أبداً القواعد الصارمة أعلاه (١-١٢).
