...
12. خذ بعين الاعتبار مدة الخطة المعتمدة (plan_duration ضمن generation_request — القيمة والوحدة، مثل "أسبوع" أو "شهر"). هذا القالب يمثّل يوماً نموذجياً يتكرر طوال هذه المدة، فكلما طالت المدة (شهر فأكثر) كن أكثر حرصاً على اقتراح بدائل متنوعة (alternative_text) لكل صنف حتى لا يضطر المريض لتناول نفس الطبق بالضبط يومياً لفترة طويلة؛ للمدد القصيرة (يوم أو أسبوع) التنويع أقل إلحاحاً. إذا كانت المدة طويلة مع عجز حراري ملحوظ، أضف تحذيراً في warnings يوصي بإعادة تقييم الوزن/الخطة قبل نهاية المدة.
