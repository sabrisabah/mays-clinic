...
def _create_plan_with_meals(patient, data, created_by, version=1, parent_plan=None):
    ...
    for i, meal_type in enumerate(MEAL_TYPES_ORDER):
        ...
        _apply_default_snack(meal)
    return plan


def _apply_default_snack(meal):
    ...
    schemas.AI_SELECTABLE_MEAL_TYPES). Instead every brand-new plan starts
    with whatever single clinic-wide default the doctor configured once in
    /admin (NutritionAISettings.default_snack1_food/quantity, likewise for
    snack2) — same item/quantity for every patient. Left blank in settings
    (food=None) means exactly the same empty سناك1/سناك2 row this app
    always had before this feature existed. The item this creates is a
    completely ordinary MealItem afterward — freely editable/removable per
    patient through the normal manual meal-item builder, exactly like any
    item a doctor adds by hand."""
    if meal.meal_type not in ("سناك1", "سناك2"):
        return
    settings_row = NutritionAISettings.get_solo()
    food = settings_row.default_snack1_food if meal.meal_type == "سناك1" else settings_row.default_snack2_food
    quantity = settings_row.default_snack1_quantity if meal.meal_type == "سناك1" else settings_row.default_snack2_quantity
    if not food or not quantity:
        return
    MealItem.objects.create(
        meal=meal, food=food, quantity=quantity, unit=food.unit,
