...
def _create_plan_with_meals(patient, data, created_by, version=1, parent_plan=None):
    ...
    for i, meal_type in enumerate(MEAL_TYPES_ORDER):
        meal = Meal.objects.create(plan=plan, meal_type=meal_type, order=i)
        _sync_auto_snack(meal, plan)
    return plan


def _sync_auto_snack(meal, plan):
    """سناك1/سناك2 are no longer generated by the AI at all (doctor
    request — snacks kept coming back inconsistent from AI generation; see
    schemas.AI_SELECTABLE_MEAL_TYPES). Instead every plan gets the single
    clinic-wide default food the doctor configured once in /admin
    (NutritionAISettings.default_snack1_food, likewise for snack2) — but its
    QUANTITY is not fixed: it's computed as a percentage
    (default_snack1_calorie_pct) of THIS plan's own calorie_target, so the
    same food scales to a different quantity per patient (5% of a
    2000-kcal target vs. 5% of a 1500-kcal target). Because the calorie
    target usually isn't known yet the instant a plan is created (the
    doctor fills it in afterward), this is called both at plan creation
    (_create_plan_with_meals — a no-op until calorie_target is set) AND
    every time the plan is saved with a calorie_target
    (NutritionPlanDetailView.put), so the quantity stays in sync as the
    target changes.

    Never touches a meal where the doctor already owns an item — the
    moment she edits the auto item's food/quantity herself
    (MealItemDetailView.put) it flips MealItem.is_auto_snack to False and
    becomes a completely ordinary, manually-owned item this function will
    never again create, update, or delete."""
    if meal.meal_type not in ("سناك1", "سناك2"):
        return
    settings_row = NutritionAISettings.get_solo()
    if meal.meal_type == "سناك1":
        food, pct = settings_row.default_snack1_food, settings_row.default_snack1_calorie_pct
    else:
        food, pct = settings_row.default_snack2_food, settings_row.default_snack2_calorie_pct
    if not food or not pct or not plan.calorie_target:
        return
    if meal.items.exclude(is_auto_snack=True).exists():
        return  # doctor already owns this slot with her own item(s) — never touch it
    if not food.calories_per_unit:
        return
    quantity = round((plan.calorie_target * pct / 100.0) / food.calories_per_unit, 1)
    if quantity <= 0:
        return
    values = dict(
        food=food, custom_food_name="", quantity=quantity, unit=food.unit,
        calories=food.calories_per_unit * quantity,
        protein=food.protein_per_unit * quantity,
        carbs=food.carbs_per_unit * quantity,
        fat=food.fat_per_unit * quantity,
        patient_visible=True, is_auto_snack=True,
    )
    auto_item = meal.items.filter(is_auto_snack=True).first()
    if auto_item:
        for field, value in values.items():
            setattr(auto_item, field, value)
        auto_item.save()
    else:
        MealItem.objects.create(meal=meal, order=0, **values)
...
class NutritionPlanDetailView(APIView):
    ...
    def put(self, request, patient_id, plan_id):
        ...
        if "calorie_target" in data:
            # Keep the percentage-of-calories سناك1/سناك2 defaults in sync
            # with whatever target this save just set — see
            # _sync_auto_snack's docstring. No-op for any slot the doctor
            # has already made her own.
            for meal in plan.meals.filter(meal_type__in=("سناك1", "سناك2")):
                _sync_auto_snack(meal, plan)
            ...
            for meal in plan.meals.all():
                ...
                for item in meal.items.all():
                    MealItem.objects.create(
                        ...
                        is_auto_snack=item.is_auto_snack,
                    )
            plan = new_plan
        else:
...
class MealItemDetailView(APIView):
    ...
    def put(self, request, patient_id, plan_id, meal_id, item_id):
        ...
        _require_draft(plan)
        if item.is_auto_snack and any(k in request.data for k in ("quantity", "food", "custom_food_name")):
            # Doctor is deliberately editing what she's served/quantity for
            # a system-generated سناك1/سناك2 item — it stops being
            # auto-managed from here on (_sync_auto_snack will never touch
            # it again, even if the plan's calorie_target changes later).
            item.is_auto_snack = False
            item.save(update_fields=["is_auto_snack"])
